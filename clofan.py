...
# # Generar una copia de trabajo
# df = df_original_citas.copy()

# # Convertir Fecha a datetime format
# df['FECHA'] = pd.to_datetime(df['FECHA'])
# 
# # Combinar 'FECHA' y 'HORA' para crear 'FECHA_HORA'
# df['Start'] = pd.to_datetime(df['FECHA'].dt.strftime('%d-%m-%y') + ' ' + df['HORA'], format='%d-%m-%y %H:%M')
# 
# # Crea una nueva columna sumando DURACION con Start
# df['DURACION'] = pd.to_timedelta(df['DURACION'] + ':00')
# df['Finish'] = df['Start'] + df['DURACION']
# df['DIA_SEM'] = df['FECHA'].dt.weekday
# df['MES_AÑO'] = df['FECHA'].dt.strftime('%m-%Y')
...
# print("Fecha minima:", df['Start'].min())
# print("Fecha Maxima:", df['Start'].max())

# ## 3. Adecuar Disponibilidad de Equipos
...
# # Convertir formato a str para posterior manipulacion
# df_equipos['HORA INICIO'] = df_equipos['HORA INICIO'].astype(str)
# df_equipos['HORA FIN'] = df_equipos['HORA FIN'].astype(str)
# 
# # Combinar 'DIA' y 'HORA INICIO'/'HORA FIN' para crear info completa de fecha con el tipo de dia
...
# ## 4. Completar df_equipos con programacion semanal
...
#print("Minimum date:", df_prueba['Start'].min())
#print("Maximum date:", df_prueba['Start'].max())
...
# df_equipos = pd.concat([df_equipos, new_df_equipos], ignore_index=True)
# df_equipos = df_equipos[df_equipos['Start'].dt.year != 1900]
# # Display the updated df_equipos
# df_equipos = df_equipos.dropna(subset = 'Start')
# df_equipos = df_equipos.rename(columns = {'EQUIPO':'RECURSO', 'DIA':'DIA_SEM'})
...
# df_ensayo = df_completa#[(df_completa['RECURSO'] == 'OCT CIRRUS')  
#                         #| (df_completa['RECURSO'] == 'ECOGRAFO')  
#                         #| (df_completa['RECURSO'] == 'OCT SOLIX') 
#                        # | (df_completa['RECURSO'] == 'CAMPO VISUAL 1') 
#                         #| (df_completa['RECURSO'] == 'CAMPO VISUAL 2') 
#                         #| (df_completa['RECURSO'] == 'PAQUIMETRO')
#                         #]

# len(df_ensayo['RECURSO'].unique())
...
#         labels={"Start": "Hora Inicio", "Finish": "Hora Fin"}
#     )
# 
...
#     range=[
#         df_ensayo['Start'].min(),  # Specify the minimum date in the 'Start' column
#         df_ensayo['Finish'].max()  # Specify the maximum date in the 'Finish' column
#     ]
# )
# num_resources = len(df_ensayo['RECURSO'].unique())
# height_per_resource = 15  # Set the height per resource (adjust as needed)
# height = np.minimum(num_resources * height_per_resource * 20, 500)
# fig.update_layout(
#     height=height,#500  # Specify the height in pixels
#     width=1200
# )
# # Set opacity to 0.5
...
#                             usecols='A:F')
# 
#     # Display preview of the sheets
#     st.write("Vista previa de la hoja CITAS:")
#     st.dataframe(df_original_citas.head())
#     
#     st.write("Vista previa de la hoja DISP_EQUIPO:")
#     st.dataframe(df_original_equipos.head())
# 
#     # 'Procesar Data' button
#     if st.button("Procesar Data"):
#         df = preprocesamiento_datos_citas(df_original_citas)
#         df_equipos = preprocesamiento_datos_disponibilidad(df_original_equipos)
# 
...
#         
#         # Save the unique month-year values to session state
#         st.session_state.mes_año_unicos = mes_año_unicos
#         
#         # Default selection for mes-año
#         if 'mes_año' not in st.session_state:
#             st.session_state.mes_año = mes_año_unicos[0]
#             
#         #Dropdown widget to select 'MES_ANO'
#         mes_año = st.selectbox('Seleccionar Mes-Año', mes_año_unicos, index=mes_año_unicos.tolist().index(st.session_state.mes_año))
# 
#         # Update session state when the select box value changes
#         st.session_state.mes_año = mes_año
# 
#         
#         df_equipos, df_prueba = construir_programacion_completa(mes_año, df, df_equipos)
#         df_completa = preprocesamiento_datos_final(df_equipos, df_prueba)
...
# 
#         # Save unique resources to session state
#         st.session_state.unique_resources = unique_resources
# 
#         # Default selection for resources
...
#         )
# 
#         # Update session state when the multiselect value changes
