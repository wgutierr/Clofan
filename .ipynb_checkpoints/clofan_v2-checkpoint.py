...
#    styled_df = df_schedule.style.map(formato_colores).apply(color_columna_hora, subset=['HORA'])
    ...
    st.dataframe(df_schedule.style.applymap(lambda x: "background-color: black; color: white" if isinstance(x, str) and x.startswith("CITA AGENDADA") else ""),hide_index=True)
